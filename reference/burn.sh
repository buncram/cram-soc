...
md5sum $TARGET_DIR/$CSR_CSV
md5sum $TARGET_DIR/$EC_IMAGE

if [ $UPDATE_KERNEL -eq 1 ]
then
  echo "Burning firmware"
  sudo wishbone-tool --csr-csv $TARGET_DIR/$CSR_CSV --load-name $TARGET_DIR/$KERNEL_IMAGE --load-address 0x500000 --load-flash
fi
if [ $UPDATE_FPGA -eq 1 ]
then
  echo "Burning FPGA image"
  sudo wishbone-tool --csr-csv $TARGET_DIR/$CSR_CSV --load-name $TARGET_DIR/$FPGA_IMAGE --load-address 0x0 --load-flash
