...
impl Engine512 {
        ...
        let ret : Engine512 =
            match which {
                Sha512Type::Sha512 => unsafe {
                    Engine512 {
                        p: betrusted_pac::Peripherals::steal(),
                        config: Sha512Config::ENDIAN_SWAP | Sha512Config::DIGEST_SWAP,
                        state: [0; 8],
                    }
                },
                Sha512Type::Sha512Trunc256 => unsafe {
                    Engine512 {
                        p: betrusted_pac::Peripherals::steal(),
                        config: Sha512Config::ENDIAN_SWAP | Sha512Config::DIGEST_SWAP | Sha512Config::SHA512_256,
                        state: [0; 8],
                    }
                },
            };
        // commit the config + enable
        unsafe{ ret.p.SHA512.config.write(|w|{ w.bits(ret.config.bits() | Sha512Config::SHA512_EN.bits()) }); }
    ...
    fn finish(&mut self) {
        ...
        unsafe{ self.p.SHA512.config.write(|w|{ w.bits(self.config.bits()) }); } // SHA512_EN is missing from the config bits, so this resets the unit
    }
    ...
    fn reset(&mut self, which: Sha512Type) {
        ...
        // commit the config + enable
        unsafe{ self.p.SHA512.config.write(|w|{ w.bits(self.config.bits() | Sha512Config::SHA512_EN.bits()) }); }
        // the "hash_start" bit puts us in a mode ready to auto-accept digest data
