...
sudo apt install help2man flex bison
...
### Install GTKWave
sudo apt-get install libjudy-dev tcl-dev libbz2-dev liblzma-dev libgconf2-dev libgtk2.0-dev tk-dev gperf gtk2-engines-pixbuf
git clone https://github.com/bunnie/gtkwave.git
cd gtkwave
git checkout udp-send
cd gtkwave3-gtk3
./autogen.sh
...
ln -s /tools/xpack-riscv-none-elf-gcc-14.2.0-3 riscv-none-elf

cargo install rustfilt

Add `export PATH="/tools/riscv-none-elf/bin:$PATH"` to ~/.bashrc

### Instal Repo & Dependencies
...
#### Dummy dependency - this needs to be fixed eventually
cd ~/code
mkdir xous-cramium
cd xous-cramium
dd if=/dev/zero of=simspi.init bs=1024 count=1

#### Xous-core contains libraries needed by nto-tests
cd ~/code
...
#### View the waveform
gtkwave build/sim/gateware/sim.fst -u 127.0.0.1:6502 &

The -u option allows you to use the `codezoom.py` script, if necessary.
