...
module sce_tb_rsa_little_endian();
...
//        .clkpke(clk),
  ...
  //  ==
    ...
    `maintest( sce_tb_rsa_little_endian, sce_tb_rsa_little_endian)
        resetn = 0;
        #( 2 `US );
        resetn = 1;
        #( 2 `US );
...
//RSAME 65
     // Design does not support 64 bit, but some cases pass
    ...
    #( 10 `US );
        ...
        membuf[0:2] = {32'h00000bcb,32'hc259fee0, 32'h1};memwr(SEGADDR_PCON,3);
        //membuf[0:2] = {32'h01000000,32'he0fe59c2, 32'hcb0b0000};memwr(SEGADDR_PCON,3);

        //H, to fill the remaining position, need to summarize the pattern, H = 1 << NLen
        membuf[0:2] = {32'h00000000,32'h00000000,32'h2};memwr(SEGADDR_PCON+3,3);
        sfrwr( PKE_crfunc, 32'h11); //RSAINIT
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);

         // RSAME
        // R1
            membuf[0:2] ={32'h95286f13,32'h95285c51, 32'h1};memwr(SEGADDR_PIB, 3);
        // E
        membuf[0:2] ={32'hb1ca1a4b,32'hb1ca19a9, 32'h1};memwr(SEGADDR_PKB, 3);

        sfrwr( PKE_crfunc, 32'h13 ); // RSAME
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
        $write("\n Reading RSAME 65 bit clock count\n");
        sfrrd(GLB_tickcnt);
        $write("\n Reading RSAME 65 bit result\n");
        memrd(SEGADDR_POB, 2);//refbuf[0:1] = {32'h41f4cca4,32'h7479d8cf}; checkref(2);

        #( 10 `US );

        $display("\n@I:: RSAME 65 little endian");
        sfrwr( GLB_suben,'1 );
        sfrwr( PKE_optnw, 32'd65);
        sfrwr( PKE_optew, 32'd65);
        ...
        //H, to fill the remaining position, need to summarize the pattern, H = 1 << NLen
        membuf[0:3] = {32'h00000000,32'h02000000,32'h0,32'h0};memwr(SEGADDR_PCON+3,4);
        sfrwr(PKE_optltx, 32'h1f);
        sfrwr( PKE_crfunc, 32'h11); //RSAINIT
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);

         // RSAME
        // R1
            membuf[0:2] ={32'h01000000,32'h515c2895, 32'h136f2895};memwr(SEGADDR_PIB, 3);
        // E
        membuf[0:2] ={32'h01000000,32'ha919cab1, 32'h4b1acab1};memwr(SEGADDR_PKB, 3);
        ...
        sfrrd(GLB_tickcnt);
        $write("\n Reading RSAME 65 bit result\n");
        memrd(SEGADDR_POB, 3);//refbuf[0:1] = {32'h41f4cca4,32'h7479d8cf}; checkref(2);
...
// RSA4096 big endian
   ...
   //X
    ...
    // Y
        ...
        sfrwr( PKE_crfunc, 32'h12 ); // RSAMM
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
        $write("\n Reading 4096 bit RSAMM big endian result\n");
        memrd(SEGADDR_POB, 128);
...
//RSAMM 128
     /*
        ...
        //N the modulo for RSA, product of two large prime
        membuf[0:3] = {32'h1,32'h0,32'hbeef9527, 32'hc259feeb};
        memwr(SEGADDR_PCON,4);

        //H, to fill the remaining position, need to summarize the pattern, H = 1 << NLen
        membuf[0:4] = {32'h00000000,32'h00000000,32'h00000000,32'h00000000,32'h1};
            memwr(SEGADDR_PCON+4,5);
        sfrwr( PKE_crfunc, 32'h11); //RSAINIT
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
    // X
        membuf[0:3] ={32'h92345678,32'haeebd162, 32'h0, 32'h0};
    // Y
        membuf[4:7] ={32'haeebd162,32'h92345678,32'h0,32'h0};
        memwr(SEGADDR_PIB, 8);
        sfrwr( PKE_segptr_PIB1, 'd4);
        sfrwr( PKE_crfunc, 32'h12 ); // RSAMM
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
        $write("\n Reading 128 bit RSAMM big endianresult\n");
        memrd(SEGADDR_POB, 4);
...
//RSAMM 128
    /*
        ...
        sfrwr( PKE_crfunc, 32'h12 ); // RSAMM
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
        $write("\n Reading 128 bit RSAMM\n");
        memrd(SEGADDR_POB, 4);



        $display("\n@I:: RSAMM 128 big endian");
        sfrwr( GLB_suben,'1 );
        sfrwr( PKE_optnw, 32'd128);
        sfrwr( PKE_optew, 32'd128);

        //N the modulo for RSA, product of two large prime
            membuf[0:3] = {32'hEBFE59C2,32'h2795EFBE,32'h00000000, 32'h01000000};
            memwr(SEGADDR_PCON,4);

        //H, to fill the remaining position, need to summarize the pattern, H = 1 << NLen, divisible by 64
            membuf[0:5] = {32'h00000000,32'h01000000,32'h00000000,32'h00000000,32'h0,32'h00000000};
            memwr(SEGADDR_PCON+4,6);

            sfrwr(PKE_optltx, 32'h1f);  //endian setting
            sfrwr( PKE_crfunc, 32'h11); //RSAINIT
            sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
        // X
            membuf[0:3] ={32'h0,32'h0, 32'h62D1EBAE, 32'h78563492};
        // Y
            membuf[4:7] ={32'h0,32'h0,32'h78563492, 32'h62D1EBAE};
            memwr(SEGADDR_PIB, 8);

            sfrwr( PKE_segptr_PIB1, 'd4);
            sfrwr( PKE_crfunc, 32'h12 ); // RSAMM
            sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
            $write("\n Reading 128 bit RSAMM big endian result\n");
            memrd(SEGADDR_POB, 4);
       */
//RSAMM 64
     /*
...
//RSAMM 64 Little endian
        ...
        //N the modulo for RSA, product of two large prime
       // membuf[0:1] = {32'h000003D9,32'h8fffffbe};
          membuf[0:1] = {32'hbeffff8f,32'hd9030000};
        memwr(SEGADDR_PCON,2);

        //H, to fill the remaining position, need to summarize the pattern, H = 1 << NLen
        membuf[0:2] = {32'h01000000,32'h00000000,32'h0};
            memwr(SEGADDR_PCON+2,3);
            sfrwr(PKE_optltx, 32'h1f);
        sfrwr( PKE_crfunc, 32'h11); //RSAINIT
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
    // X
        membuf[0:1] ={32'h0,32'h0b000000};

    // Y
        membuf[2:3] ={32'h0,32'h07000000};
     //   big_to_little();
        memwr(SEGADDR_PIB, 4);
        sfrwr( PKE_segptr_PIB1, 'h2);
        sfrwr( PKE_crfunc, 32'h12 ); // RSAMM
...
// RSA MA 64
    ...
    #( 1 `US);
        ...
        // Y
            membuf[2:3] =  {32'h3,32'h0};
            memwr(SEGADDR_PIB, 4);
            sfrwr( PKE_segptr_PIB1, 'h2);
            sfrwr( PKE_crfunc, 32'h19 ); // RSAMA
            sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
            $write("\n Reading 64 bit RSAMA result\n");
            memrd(SEGADDR_POB, 2);refbuf[0:1] = {32'h4,32'h0};checkref(2);
    ...
    #( 1 `US);
        ...
        sfrwr( PKE_optmask, 32'h1);
        sfrwr(PKE_optltx, 32'h0f);
        sfrwr( PKE_crfunc, 32'h11); //RSAINIT
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
        // X
               membuf[0:1] = {32'h000003D5,32'h8fffffbe};
        // {membuf[0], membuf[1]} =   {membuf[1], membuf[0]};

        // Y
            membuf[2:3] =  {32'h3,32'h0};
         //       {membuf[2], membuf[3]} =   {membuf[3], membuf[2]};
             big_to_little();
            memwr(SEGADDR_PIB, 4);
