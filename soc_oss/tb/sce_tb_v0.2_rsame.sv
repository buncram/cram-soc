...
//        .clkpke(clk),
    ...
    assign probe_pkeram_18d = (sce_tb_rsame.dut.pke.ramadd0[8:0] == 'h18d);

  assign probe_pkeram_14a = (sce_tb_rsame.dut.pke.ramadd0[8:0] == 'h14a);
    //assign probe_pkeram_14c = (sce_tb_rsame.dut.pke.ramadd0[8:0] == 'h18c) && ;
  ...
  //  ==
    ...
    `maintest(sce_tb_rsame,sce_tb_rsame)
        resetn = 0;
        #( 2 `US );
        resetn = 1;
        #( 2 `US );
...
//ECPA
    ...
    /*
   #( 10 `US );
        $display("\n@I:: PKE ECC256r1 Point ADD");

        sfrwr( GLB_suben,'1 );
        sfrwr( PKE_optnw, 32'd255 );
        sfrwr( PKE_optew, 32'd3 );
        ...
        // K
             membuf[0:7] = {32'h5,32'h0,32'h0,32'h0,32'h0,32'h0,32'h0,32'h0};
            memwr(SEGADDR_PKB,8);

        // Q0_X,Q0_Y, I2MD
            membuf[0:7] = {32'h8f25d51a,32'hc9562d60,32'h9525a7b2,32'h692cc760,32'hfdd6dc5c,32'hc0a4e231,32'hcd6e53fe,32'h216936d3};
            membuf[8:15] = {32'h66666658,32'h66666666,32'h66666666,32'h66666666,32'h66666666,32'h66666666,32'h66666666,32'h66666666};
     $display("\n@I:: First trans\n");
             memwr(SEGADDR_PIB,16);
            sfrwr( PKE_crfunc, 32'h24 ); // I2MD
            sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
      ...
      #( 10 `US );
        // Q1_X,Q1_Y, I2MD
            ...
            membuf[0:7] = {32'h2843ce0e,32'h83c5a14e,32'h15d7a45f,32'h080d8e45,32'h1833e7ac,32'h3d043b7d,32'h9f5a046c,32'h36ab384c};
            membuf[8:15] = {32'h6af8a3c9,32'h0e5f46ae,32'h64385156,32'h97390f51,32'hc9a21f56,32'h1da25ee8,32'h092329c2,32'h2260cdf3};
  ...
  $display("\n@I:: Second trans\n");
        ...
        #( 1 `US );
            ...
            sfrwr(SDMA_ichcr_transize,32'd32);
            sfrwr(SDMA_ichcr_rpstart, 32'd0);
            sfrwr(SDMA_ichcr_wpstart,32'd128+'d32);
            sfrwr(SDMA_chstart_ar,32'h5a);
            #( 10 `US );
            memrd(SEGADDR_PIB+128,64);

        $display("\n@I:: Result point\n");
            sfrwr( PKE_segptr_PIB1, 'd128 ); 
            sfrwr( PKE_segptr_PIB0, 'd128+'d32 ); 
...
// RSA MA 64
    ...
    #( 1 `US);
        ...
        sfrwr( PKE_ar, 32'h5a ); sfrwait(PKE_fr, 32'h1, 32'h1); sfrwr(PKE_fr, 32'h1);
        // X
               membuf[0:1] = {32'h000003D5,32'h2fffffbe};
        // Y
...
// RSA MA 4096
    ...
    #( 1 `US);
        ...
        membuf[121:128] = {32'h0,32'h0,32'h2,32'h0,32'h0,32'h1,32'h0,32'h0};
