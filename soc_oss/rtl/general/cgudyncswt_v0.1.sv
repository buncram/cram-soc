...
(
    ...
    cgudyncswt_sync sync00 ( .clk( clk1 ), .sin( clksel_s00 ), .sout( clksel_s01 ));
    cgudyncswt_sync sync01 ( .clk( clk0 ), .sin( clksel_s01 ), .sout( clksel_s02 ));

    cgudyncswt_sync sync10 ( .clk( clk0 ), .sin( clksel_s10 ), .sout( clksel_s11 ));
    cgudyncswt_sync sync11 ( .clk( clk1 ), .sin( clksel_s11 ), .sout( clksel_s12 ));
...
module cgudyncswt_sync( 
    ...
    always@(posedge clk) sreg <= { sin, sreg[0:scnt-2] };
