...
logic   [31:0] ramrdata;
logic [AW-3:0] ramaddr;
logic    [3:0] ramwen;
logic   [31:0] ramwdata;
logic          ramcs; 
logic          hsel;

assign hsel = ( ahbs.haddr[31:16] == 16'h1c00 );

ahb_to_sram #( .AW (AW) )u0(
  .HCLK           (clk),   
  .HRESETn        (resetn),    
  .HSEL           (hsel),   
  .HREADY         (ahbs.hreadym),   
  ...
  bit [0:WC-1][WW-1:0] ramdata;

  always@(posedge clk) if(ramcs & ramwen[0]) ramdata[ramaddr][ 7: 0] <= ramwdata[ 7: 0];
  always@(posedge clk) if(ramcs & ramwen[1]) ramdata[ramaddr][15: 8] <= ramwdata[15: 8];
  always@(posedge clk) if(ramcs & ramwen[2]) ramdata[ramaddr][23:16] <= ramwdata[23:16];
  always@(posedge clk) if(ramcs & ramwen[3]) ramdata[ramaddr][31:24] <= ramwdata[31:24];
  `thereg(ramrdata) <= ramcs &  ( ramwen == 0 ) ? ramdata[ramaddr] : ramrdata;
