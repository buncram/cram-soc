...
module scedmachnl_ahbs #(
    ...
    logic [2:0] cr_ahbwropt_bitwise;
    logic cr_ahbrdopt_xor;
    logic cr_ahbrdopt_bitwise;

// cr_opt
// ■■■■■■■■■■■■■■■ 

    assign cr_ahbwropt_bitwise = cr_opt[2:0];
    assign cr_ahbrdopt_xor = cr_opt[3];
    assign cr_ahbrdopt_bitwise = cr_opt[4];

// ahb
// ■■■■■■■■■■■■■■■ 

    assign ahbs_cpvld = ahbs.hsel & ahbs.hreadym & ahbs.htrans[1] & ahbs.hready ;
    ...
    assign ahbshaddr_local = ahbs.haddr[AW-1:0] - BA;
    `theregrn( ahbshaddr_dp ) <= ahbs_cpvld ? ahbshaddr_local : ahbshaddr_dp;

    scedmachnl_addr2seg #(.SEGCNT(SEGCNT),.SEGCFGS(SEGCFGS)) ut( .addr(ahbshaddr_local), .segid( ahbs_segid ));
...
// ■■■■■■■■■■■■■■■ 
    ...
    assign chnli_thecfg.opt_ltx = cr_ahbwropt_bitwise | '0;
    assign chnli_thecfg.opt_xor = cr_ahbrdopt_xor;
    assign chnli_thecfg.opt_cmpp = '0;
    assign chnli_thecfg.opt_prm = '0;

    assign chnlo_thecfg.chnlid = 'd1;
    assign chnlo_thecfg.rpsegcfg = ahbs_segcfgreg;
    ...
    assign chnlo_thecfg.opt_ltx = cr_ahbrdopt_bitwise  | '0;
...
module scedmachnl_axim  #(
    ...
    localparam PM_PROT = 'h2;
            ...
            // [1] 0:secure access
            // [2] 0:data | 1:instr
    logic [2:0] cr_axiwropt_bitwise;
    logic cr_axirdopt_xor;
    logic cr_axirdopt_bitwise;
...
// ■■■■■■■■■■■■■■■ 
    ...
    assign cr_axiwropt_bitwise = cr_opt[2:0];
    assign cr_axirdopt_xor = cr_opt[3];
    assign cr_axirdopt_bitwise = cr_opt[4];
...
// AR/R chnl
    ...
    assign aximrpres.segrdatvld = axim.rvalid & ardp;
...
// AW/W/B chnl
    ...
    `theregrn( wcpreg ) <= wcp & wrdy ? 'b0 : aximwpreq.segwr ? 'b1 : wcpreg;
...
// ■■■■■■■■■■■■■■■ 
    ...
    assign axim.araddr  = cr_axaddrstart + aximprt;
    assign axim.arid    = PM_AXID;
    ...
    assign axim.aruser  = cr_segid;

    assign axim.rready = ardp;

    assign axim.awvalid  = awcp & axunmask;
    assign axim.awaddr   = cr_axaddrstart + aximprt;
    assign axim.awid     = PM_AXID;
    ...
    assign axim.awuser   = cr_segid;

    assign axim.wvalid = wcp;
    assign axim.wid    = PM_AXID;
...
// ■■■■■■■■■■■■■■■ 
    ...
    assign chnli_thecfg.opt_ltx = cr_axiwropt_bitwise;
    assign chnli_thecfg.opt_xor = cr_axirdopt_xor;
    assign chnli_thecfg.opt_cmpp = '0;
    assign chnli_thecfg.opt_prm = '0;
    ...
    assign chnlo_thecfg.opt_ltx = cr_axirdopt_bitwise | '0;
    assign chnlo_thecfg.opt_xor = '0;
    assign chnlo_thecfg.opt_cmpp = '0;
    assign chnlo_thecfg.opt_prm = '0;
...
always_comb begin
    ...
    if( addr >= SEGCFGS[SEGCNT].segaddr && addr < ( SEGCFGS[SEGCNT].segaddr + SEGCFGS[SEGCNT].segsize ) )
        segid = SEGCNT;
end
