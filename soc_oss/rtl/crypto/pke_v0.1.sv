...
// ■■■■■■■■■■■■■■■
    ...
    localparam [7:0] SEGID_PCON = scedma_pkg::SEGID_PCON ;
    localparam [7:0] SEGID_PKB  = scedma_pkg::SEGID_PKB  ;
    localparam [7:0] SEGID_PIB  = scedma_pkg::SEGID_PIB  ;
    localparam [7:0] SEGID_PSIB = scedma_pkg::SEGID_PSIB ;
    localparam [7:0] SEGID_POB  = scedma_pkg::SEGID_POB  ;
    localparam [7:0] SEGID_PSOB = scedma_pkg::SEGID_PSOB ;
...
//    localparam PIR_RSAMI   = 8'h18 ;
    ...
    localparam scedma_pkg::segcfg_t RSASEG_H      = '{ segid:'0, segtype:ST_NONE, ramsel:'0, segaddr: 'h500, segsize: 'd128, isfifo:'0, isfifostream:0, fifoid:'0 };
    ...
    localparam scedma_pkg::segcfg_t INVSEG_D      = '{ segid:'0, segtype:ST_NONE, ramsel:'0, segaddr: 'h510/*##*/, segsize: 'd128/*#*/, isfifo:'0, isfifostream:0, fifoid:'0 };
...
// ■■■■■■■■■■■■■■■
    ...
    assign apbx.prdata = '0
                        ...
                        | sfr_optnw.prdata32 | sfr_optew.prdata32 | sfr_optmask.prdata32 | sfr_segptr.prdata32
                        ;
...
// ■■■■■■■■■■■■■■■
    ...
    `theregrn( opt_n0[31:0]  ) <= opt_n0lwr ? chnl_wpreq.segwdat : opt_n0[31:0];
    `theregrn( opt_n0[63:32] ) <= opt_n0hwr ? chnl_wpreq.segwdat : opt_n0[63:32];

    assign opt_n0lwr = (( mfsm == MFSM_LD_N ) | ( mfsm == MFSM_LD_P )) & ( chnli_wpreq.segptr == 0 ) & chnli_wpreq.segwr ;
    assign opt_n0hwr = (( mfsm == MFSM_LD_N ) | ( mfsm == MFSM_LD_P )) & ( chnli_wpreq.segptr == 0 ) & chnli_wpreq.segwr ;
        ...
        case (cr_func)
            ...
            PIR_RSAINIT:
                case( mfsm )
                    MFSM_IDLE:
                                                mfsmnext = MFSM_LD_N;
                    MFSM_LD_N : begin
                                                mfsmnext = MFSM_LD_H;       mfsmdone = chnli_done;
                        end
                    MFSM_LD_H : begin
                                                mfsmnext = MFSM_J0;         mfsmdone = chnli_done;
                        end
                    MFSM_J0 : begin
                                                mfsmnext = MFSM_PF;         mfsmdone = pcore_done;
                        end
                    MFSM_PF : begin
            ...
            PIR_EDM2I:
                case( mfsm )
                    ...
                    MFSM_PF : begin
                                                mfsmnext = MFSM_WB_Q0I;     mfsmdone = pcore_done;
                        end
                    MFSM_WB_Q0I : begin
                                                mfsmnext = MFSM_DONE;       mfsmdone = chnlo_done;
                        end
            ...
            PIR_X25519:
                case( mfsm )
                    ...
                    MFSM_PF : begin
                                                mfsmnext = MFSM_WB_Q0X;     mfsmdone = pcore_done;
                        end
                    MFSM_WB_Q0X : begin
                                                mfsmnext = MFSM_DONE;       mfsmdone = chnlo_done;
                        end
...
// ■■■■■■■■■■■■■■■
    ...
    `theregrn( pcore_start ) <= mfsmtog & ( mfsm == MFSM_PF  );

    assign pcore_en = ( mfsm == MFSM_PF );
    assign opt_nw32 = opt_nw[12:5] + |opt_nw[4:0];

    assign opt_nwb = opt_nw + 1;
    assign opt_nw32b = opt_nwb[12:5] + |opt_nwb[4:0];

    logic [7:0] pkeir;
    assign pkeir = MFSM_J0 ? 8'h20 : cr_func;
    ...
    // 2KB x 2, 64bit data, 8 bit addr for each.
...
//    segcfg_t  ecpsegwpcfg, ecpsegrpcfg;
    logic [RAW:0] ecpsegsize, ecpsegwpaddr, ecpsegrpaddr, ecpsegwpaddrs, ecpsegrpaddrs, ecpsegptr ;
    logic ecpsegwpaddrset, ecpsegrpaddrset;
    logic [1:0] ecpsegwpaddrsel, ecpsegrpaddrsel, ecpsegidx;

    assign ecpsegwpcfg = '{ segid:'0, segtype:ST_NONE, ramsel:'0, segaddr: ecpsegwpaddr, segsize: ecpsegsize, isfifo:'0, isfifostream:0, fifoid:'0 };
    assign ecpsegrpcfg = '{ segid:'0, segtype:ST_NONE, ramsel:'0, segaddr: ecpsegrpaddr, segsize: ecpsegsize, isfifo:'0, isfifostream:0, fifoid:'0 };
    ...
    `theregrn( ecpsegwpaddr ) <= ecpsegwpaddrset ? ecpsegwpaddrs[ecpsegidx] : ecpsegwpaddr;
    `theregrn( ecpsegrpaddr ) <= ecpsegrpaddrset ? ecpsegrpaddrs[ecpsegidx] : ecpsegrpaddr;
    ...
    assign ecpsegrpaddrs = ( ecpsegrpaddrsel == 0 ) ? ECPSEGADDR_Q0 :
                           ...
                           ( ecpsegrpaddrsel == 2 ) ? ECPSEGADDR_P0 :
                                                      ECPSEGADDR_P1 ;
    `theregrn( ecpsegidx ) <= mfsmtog ? '0 : ( ecpsegptr == opt_nw32 - 1 ) & ( chnl_ramwr ) ? ecpsegidx + 1 : ecpsegidx;
    `theregrn( ecpsegptr ) <= mfsmtog ? '0 : ( chnl_ramwr ) ? (( ecpsegptr == opt_nw32 - 1 ) ? '0 : ecpsegptr + 1 ) : '0;
        ...
        case(mfsm)
            ...
            MFSM_LD_Q0I    :
                begin
                    ...
                    ecpsegwpen = 1'b1;
                    ecpsegwpaddrsel_i = 0;
                    chnli_cfg.transsize = opt_nw32 * 2;
                    chnli_en = '1;
                end
            MFSM_LD_Q1I    :
                begin
            ...
            MFSM_LD_CON1   :
                begin
                    chnli_cfg.rpsegcfg = '0;//SEG_R0;//####
                    chnli_cfg.rpptr_start = '0;
                    chnli_cfg.wpsegcfg = ECCSEG_CON1;
                    chnli_cfg.transsize = opt_nw32;
                    chnli_dummy_con1 = 1;
                    chnli_en = '1;
                end
...
// ■■■■■■■■■■■■■■■
    ...
    assign chnl_ramrd  = chnli_wpreq.segrd | chnlo_rpreq.segrd | chnlx_rpreq.segrd | chnlx_wpreq.segrd;
    assign chnl_ramwr  = chnli_wpreq.segwr | chnlo_rpreq.segwr | chnlx_rpreq.segwr | chnlx_wpreq.segwr;
    ...
    assign chnl_ramsel0 = ( chnl_ramadd[RAW] == 'h0 ) ;
    assign chnl_ramsel1 = ( chnl_ramadd[RAW] == 'h1 ) ;
    ...
    assign chnl_ramadd0 = chnl_ramadd[RAW-1:1];
    assign chnl_ramadd1 = chnl_ramadd[RAW-1:1];

    `theregrn( ramsel0reg ) <= chnl_ramsel0;
    `theregrn( chnl_ramdatselreg ) <= chnl_ramadd[0];
    assign pramres.segready = '1;  // pram always ready
    assign pramres.segrdat  = ramsel0reg ? ramrdat0 : ramrdat1;
    ...
    assign ramrd0 = pcore_en ? pcore_ramrd0 : chnl_ramrd0;
    assign ramrd1 = pcore_en ? pcore_ramrd1 : chnl_ramrd1;
    assign ramwr0 = pcore_en ? {8{pcore_ramwr0}} : chnl_ramwr0;
    assign ramwr1 = pcore_en ? {8{pcore_ramwr1}} : chnl_ramwr1;
