...
// ■■■■■■■■■■■■■■■
    ...
    apb_cr #(.A('h18), .DW(4))       sfr_optltx     (.cr(cr_optltx), .prdata32(),.*); 
...
// ■■■■■■■■■■■■■■■
    ...
    bit [1:0] aesir;
    assign aesir = ( cr_func == AF_ENC ) ? 2'h2 :
                   ( cr_func == AF_DEC ) ? 2'h3 : '0;
...
// ■■■■■■■■■■■■■■■
    ...
    segcfg_t aesseg_cfg_for_ld_iv;

    assign ld_iv_to_i = ld_d_to_iv;
    assign ld_d_to_iv = (mfsm == MFSM_LD_IV) && ~(( opt_mode == MODE_ECB ) | ( opt_mode == MODE_CBC )) ;
    assign ld_iv_from_aesram = ~aes0 && (mfsm == MFSM_LD_IV) && (( opt_mode == MODE_CBC ) | ( opt_mode == MODE_OFB ) |  ( opt_mode == MODE_CFB ));

    assign aesseg_cfg_for_ld_iv = ( opt_mode == MODE_CBC ) ? (( cr_func == AF_ENC ) ? AESSEG_O : AESSEG_I ) :
        ...
        case(mfsm)
            ...
            MFSM_LD_IV      :   
                begin
                    ...
                    chnli_cfg.rpptr_start = ld_iv_from_aesram ? 0 : cr_segptrstart[PTRID_IV];
                    chnli_cfg.rpsegcfg =    ld_iv_from_aesram ? aesseg_cfg_for_ld_iv : SEG_AKEY;
                    chnli_cfg.opt_ltx =     ld_iv_from_aesram ? '0 : cr_optltx[PTRID_IV];
                    chnli_cfg.transsize = opt_d32;
                    chnli_en = '1;
                end
            default : /* default */
...
//    assign chnli_cfg.opt_ltx = '0;
    ...
    assign chnlo_cfg.rpsegcfg = AESSEG_O;
    assign chnlo_cfg.wpsegcfg = SEG_AOB;
    assign chnlo_cfg.wpptr_start = cr_segptrstart[PTRID_AOB] + segptrdyna_wr;
    assign chnlo_cfg.transsize = opt_d32;

    assign chnlo_cfg.chnlid = '0;
    assign chnlo_cfg.rpptr_start = '0;
    assign chnlo_cfg.opt_ltx = cr_optltx[PTRID_AOB];
...
// chnl instance
// ■■■■■■■■■■■■■■■
    logic chnli_aesramsel;

    assign chnli_aesramsel = ld_iv_from_aesram;
    ...
    assign chnl_rpreq.segrd    = chnl_rpreq0.segrd  & ~chnli_aesramsel ;
...
// ■■■■■■■■■■■■■■■
    ...
    assign chnl_ramadd = chnli_aesramsel ? chnl_rpreq.segaddr + chnl_rpreq.segptr :
                         chnlo_en ?  chnlo_rpreq.segaddr + chnlo_rpreq.segptr  : 
                                     chnli_wpreq.segaddr + chnli_wpreq.segptr  ; //chnli_en ?

    assign chnl_segptr = chnlo_en ?  chnlo_rpreq.segptr  : 
                                     chnli_wpreq.segptr  ; //chnli_en ?

    assign chnl_ramrd  = chnli_wpreq.segrd | chnlo_rpreq.segrd | ( chnli_aesramsel & chnl_rpreq.segrd ) ;
    ...
    assign chnl_rpres0 = chnli_aesramsel ? aramres :chnl_rpres;
