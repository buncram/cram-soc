...
// ■■■■■■■■■■■■■■■
    ...
    scedma_pkg::adr_t [0:SEGCNT-1]   cr_segptrstart;
    logic mfsm_done;
    logic hash0;
// apb
// ■■■■■■■■■■■■■■■
    ...
    assign apbx.prdata = '0
                        ...
                        | sfr_opt1.prdata32 | sfr_opt2.prdata32 | sfr_segptr.prdata32
                        ;
    ...
    apb_fr #(.A('h0c), .DW(4))      sfr_fr          (.fr({chnli_done, chnlo_done, hash_done, mfsm_done}), .prdata32(),.*);

    apb_cr #(.A('h10), .DW(16))      sfr_opt1       (.cr(cr_opt.hashcnt), .prdata32(),.*); 
    apb_cr #(.A('h14), .DW(3))       sfr_opt2       (.cr({cr_opt.ifstart,cr_opt.ifsob,cr_opt.scrtchk}), .prdata32(),.*); 

    apb_cr #(.A('h20), .DW(scedma_pkg::AW), .SFRCNT(SEGCNT) )      sfr_segptr    (.cr(cr_segptrstart), .prdata32(),.*); //## width?
