...
module sce #(
    ...
    output [INTC-1:0] intr,
    output [ERRC-1:0] err
);

// cfg

    parameter axi_pkg::xbar_rule_32_t [1:0] ADDRMAP = '{
    ...
    logic               alu_err , hash_err , pke_err , aes_err , trng_err ;
    logic [2:0]         sdma_busy;
    logic [2:0]         sdma_done;
    logic [2:0]         sdma_err; 
    logic               acenable;
...
// ■■■■■■■■■■■■■■■
    ...
    assign glb_busy = { sdma_busy, alu_busy, hash_busy, pke_busy, aes_busy, trng_busy };
    assign glb_done = { sdma_done, alu_done, hash_done, pke_done, aes_done, trng_done };
    assign glb_err  = '0;//{ sdma_err , alu_err , hash_err , pke_err , aes_err , trng_err  };

    assign mode_non = ( glb_scemode == 0 );
    assign mode_xls = ( glb_scemode == 1 );
    assign mode_sec = ( glb_scemode[1] == 1 );

    bit mode_secreg;
    `theregrn( mode_secreg ) <= mode_sec;
    `theregrn( sceuser ) <= ( mode_sec & ~mode_secreg ) ? coreuser : sceuser;
