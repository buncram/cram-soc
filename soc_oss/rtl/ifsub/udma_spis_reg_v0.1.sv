...
///////////////////////////////////////////////////////////////////////////////
//
// Description: spis configuration interface
//
///////////////////////////////////////////////////////////////////////////////
//
...
`define REG_SPIS_SETUP   5'b01001 //BASEADDR+0x24
...
module udma_spis_reg_if #(
    ...
    begin
        ...
        begin
           ...
           if (r_spis_rx_polling_en | r_spis_rx_irq_en) begin
              if(rx_valid_i & ~s_rx_valid_clr) begin
                 r_spis_rx_data       <= rx_data_i;
                 r_spis_rx_data_valid <= rx_valid_i;
              end
              else if (s_rx_valid_clr) begin
                 r_spis_rx_data       <= r_spis_rx_data;
                 r_spis_rx_data_valid <= 1'b0;
              end else begin
                 r_spis_rx_data       <= r_spis_rx_data;
                 r_spis_rx_data_valid <= r_spis_rx_data_valid;
              end
           end else begin
              r_spis_rx_data       <= r_spis_rx_data;
              r_spis_rx_data_valid <= 1'b0;
            ...
            begin
                ...
                `REG_SPIS_SETUP:
                begin
                    r_spis_div        <= cfg_data_i[31:16];
                    r_spis_en_rx      <= cfg_data_i[9];
                    r_spis_en_tx      <= cfg_data_i[8];
                    r_spis_rx_clean_fifo <= cfg_data_i[5];
                    r_spis_rx_polling_en <= cfg_data_i[4];
                    r_spis_stop_bits  <= cfg_data_i[3];
                    r_spis_bits       <= cfg_data_i[2:1];
                    r_spis_parity_en  <= cfg_data_i[0];
                end
                `REG_IRQ_EN:
                  begin
                    r_spis_err_irq_en <= cfg_data_i[1];
                    r_spis_rx_irq_en  <= cfg_data_i[0];
                  end
                endcase
            end
        end
    ...
    begin
        ...
        `REG_SPIS_SETUP:
            cfg_data_o = {r_spis_div, 6'h0, r_spis_en_rx, r_spis_en_tx, 2'h0, r_spis_rx_clean_fifo, r_spis_rx_polling_en, r_spis_stop_bits,r_spis_bits, r_spis_parity_en};
        `REG_STATUS:
        ...
        `REG_IRQ_EN:
            cfg_data_o = {30'h0, r_spis_err_irq_en, r_spis_rx_irq_en};
        `REG_VALID:
            cfg_data_o = {31'h0, r_spis_rx_data_valid};
        `REG_DATA:
          begin
             cfg_data_o     = {24'h0, r_spis_rx_data};
             s_rx_valid_clr = 1'b1;
          end
        default:
            cfg_data_o = 'h0;
