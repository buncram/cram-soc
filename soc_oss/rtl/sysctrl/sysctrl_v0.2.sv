...
//        output logic fclksub,
    ...
    bit   [0:5][7:0]    cfgfd, cfgfdlp, cgufd;
    bit   [0:5][15:0]   cfgfdcr;
    bit [7:0]           clk1Mcnt;
    bit [9:0]           clk32kcnt;
    bit [3:0]           fsvld;
    bit [3:0][15:0]     fsfreq;
...
//        assign clk32m = clkxtl;
        ...
        assign ipc_osc = osc_osc32m_cfg;
        OSC_32M osc32M ( .EN(ipc_oscen), .CFG(osc_osc32m_cfg),      .CKO( clkosc  ) );
...
//        .GEARLMT = 2**FDW
        ...
        /*output  logic [0:OCNT-1]    */.clkout             ({fclk,aclk,hclk,iclk,pclk,aoclk}),
    ...
    logic clksys1mreg;
    `theregfull( clksys, chipresetn, clksys1mcnt, '0 ) <= clksys1mcnt + 1;
    `theregfull( clksys, chipresetn, clksys1mreg, '0 ) <= ( clksys1mcnt == '1 ) ^ clksys1mreg;
    CLKCELL_BUF buf_clksysao(.A(clksys1mreg),.Z(clksys1m));
    ...
    assign { cfgfdlp[5], cfgfd[5] } = cfgfdcr[5];
...
// ==
    ...
    assign { clkpkesel, fdpke } = cfgfdpke;
    ...
    assign clkpkeen = ( ~clkpkesel ? hclken : oclkpkeen ) & hclksubgate[1];
//    ICG fdicgpke ( .CK (clktop   ), .EN ( clkpkeen ), .SE(cmsatpg), .CKG ( clkpke ));
...
`ifdef FPGA
    `theregfull( clktop, coreresetn, clktopenin, '1 ) <= '1;
`else
    `theregfull( clktop, coreresetn, clktopenin, '1 ) <= clkcipheren ? ( clkcipherdat >= clkcipherlevel * 16 ) : '1;
`endif
...
// pad reton/off
    ...
    `theregfull( pclk, chipresetn, pad_retonreg, '0) <= ipccr[8];
    assign pad_reton = pad_retonreg;
...
//    `theregfull( clksys, pdresetn, coresleepregs_clksys, '0 ) <= { coresleepregs_clksys, 1'b1};
    ...
    assign pdresetn0 = coreresetn & ~wkupvld_async;
...
// ■■■■■■■■■■■■■■■
    ...
    assign apbx.prdata = '0
                ...
                | sfr_cgusel0.prdata32 | sfr_cgufd.prdata32 | sfr_cgusel1.prdata32 | sfr_cgufdao.prdata32 | sfr_cgufdpke.prdata32
...
// cgu cfg cr
    ...
    apb_cr #(.A('h14), .DW(8*2),
             .IV('h7f7f), .SFRCNT(5))
                                    sfr_cgufd   (.cr(cfgfdcr[0:4]),    .prdata32(),.*);
    apb_cr #(.A('h28), .DW(16), .IV('h0f0f))
                                    sfr_cgufdao   (.cr(cfgfdcr[5]),    .prdata32(),.*);

    apb_cr #(.A('h34), .DW(9), .IV('h3f))
                                    sfr_cgufdpke   (.cr(cfgfdpke),    .prdata32(),.*);
...
// ipc
    ...
    apb_sr #(.A('h9c), .DW(7)             )    sfr_ipcosc   (.sr(ipc_osc),   .prdata32(),.*);
