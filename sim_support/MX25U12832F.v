// *============================================================================================== 
// *
// *   MX25U12832F.v - 128M-BIT CMOS Serial Flash Memory
// *
...
// * Reference Doc: MX25U12832F REV.1.2,NOV.21,2019
// * Creation Date: @(#)$Date: 2019/11/25 02:52:51 $
// * Version      : @(#)$Revision: 1.8 $
// * Description  : There is only one module in this file
// *                module MX25U12832F->behavior model for the 128M-Bit flash
// *----------------------------------------------------------------------------------------------
...
// *==============================================================================================
        `define MX25U12832FM2 8SOP      // RESET#/SIO3
        //`define MX25U12832FZ 8WSON      // RESET#/SIO3
        //`define MX25U12832FMI 16SOP      // RESET#
...
// *============================================================================================== 
    ...
    parameter   tSHQZ           = 8,        // CS High to SO Float Time [ns]
                ...
                tBP             = 18_000,      //  Byte program time
                tSE             = 30_000_000,      // Sector erase time  
                tBE             = 300_000_000,      // Block erase time
                tBE32           = 150_000_000,    // Block 32KB erase time
                tCE             = 36_000,      // unit is ms instead of ns
                tPP             = 400_000,      // Program time
                ...
                tRDP            = 30_000,    //Recovery Time for Release from deep power down mode
                tCRDP           = 20,      //  CS# Toggling Time before Release from Deep Power-Down Mode 
                tDPDD           = 30_000,      //  Delay Time for Release from Deep Power-Down Mode once entering Deep Power-Down Mode Mode
                tWPS            = 18_000,     // Write Protection Select time
    ...
    parameter   tPSL            = 25_000,       // delay after page program suspend command
                ...
                tERS            = 100_000;      // latency between erase resume and next suspend
    ...
    parameter   WREN        = 8'h06, // WriteEnable   
                ...
                RDFS        = 8'h44, // Read Factory Mode Status Register 
    ...
    always @ ( posedge ISCLK or posedge CS_INT ) begin
        ...
        if ( CS_INT == 1'b0 ) begin
            ...
            if ( SI_IN_EN == 1'b1 && SO_IN_EN == 1'b1 && WP_IN_EN == 1'b1 && SIO3_IN_EN == 1'b1 ) begin
                SI_Reg[31:0] = ( CR[5] ) ? {SI_Reg[27:0], SIO3, WP, SO, SI} : {8'b0, SI_Reg[19:0], SIO3, WP, SO, SI};
            end
            else  if ( SI_IN_EN == 1'b1 && SO_IN_EN == 1'b1 ) begin
                SI_Reg[31:0] = ( CR[5] ) ? {SI_Reg[29:0], SO, SI} : {8'b0, SI_Reg[21:0], SO, SI};
            end
            else if ( SI_IN_EN == 1'b1 ) begin 
                SI_Reg[31:0] = ( CR[5] ) ? {SI_Reg[30:0], SI} : {8'b0, SI_Reg[22:0], SI};
            end

            if ( (EN4XIO_Read_Mode == 1'b1 && ((Bit == 5 && !ENQUAD && !CR[5] ) || (Bit == 23 && ENQUAD && !CR[5] ))) ) begin
                Address = SI_Reg[A_MSB:0];
                load_address(Address);
            end
            else if ( (EN4XIO_Read_Mode == 1'b1 && ((Bit == 7 && !ENQUAD &&  CR[5] ) || (Bit == 31 && ENQUAD &&  CR[5] ))) ) begin
                Address = SI_Reg[A_MSB:0];
                load_address(Address);
            end  
        end     
            ...
            `CMD_STATE: 
                begin
                    ...
                    RDFS:
                        begin 
                            if ( !DP_Mode && !WIP && Chip_EN && ~HPM_RD && !EPSUSP ) begin 
                                //$display( $time, " Enter Read Factory Status Function ..." );
                                Read_SHSL = 1'b1;
                                RDFS_Mode = 1'b1;
                            end
                            else if ( Bit == 7 )
                  ...
                  RDID:
                   ...
                   QPIID:
                    ...
                    W4READ:
                        begin
                            if ( !DP_Mode && !WIP && Status_Reg[6] && Chip_EN && ~HPM_RD && !ENQUAD ) begin
                                //$display( $time, " Enter Quad I/O Read Data Function ..." );
                                Read_SHSL = 1'b1;
                                if ( Bit == 13 ) begin
                                    Address = SI_Reg [A_MSB:0];
                                    load_address(Address);
                    ...
                    SUSP1:
                        begin
                            if ( !DP_Mode && !Secur_Mode &&  Chip_EN && ~HPM_RD && !EPSUSP && !Factory_Mode_Reg ) begin
                                if ( CS_INT == 1'b1 && Bit == 7 ) begin
...
// * ============================================================================================
    ...
    always @ ( RST_Event ) begin
        ...
        else if ( WR_WPSEL_Mode || WRLR_Mode || PASSULK_Mode || WRSPB_Mode || WRSCUR_Mode ||  PP_4XIO_Mode || PP_1XIO_Mode ) begin
            #(tREADY2_P);
        end
...
// * ============================================================================================
    ...
    always @ ( posedge RDFS_Mode ) begin
        read_fs;
    end
...
// *========================================================================================== 
    ...
    /*----------------------------------------------------------------------*/
    /*  Description: define a read factory mode status task (RDFS)                           */
    /*----------------------------------------------------------------------*/
    task read_fs;
        integer Dummy_Count;
        begin
            ...
            forever begin
                ...
                if ( CS_INT == 1'b1 ) begin
                    ...
                    SIO3_IN_EN= 1'b0;
                    if (ENQUAD)
                        {SIO3_Reg, SIO2_Reg, SIO1_Reg, SIO0_Reg, Factory_Mode_Reg} <= {Factory_Mode_Reg, Factory_Mode_Reg[7:4]};
                    else
                        {SIO1_Reg, Factory_Mode_Reg} <= {Factory_Mode_Reg, Factory_Mode_Reg[7]};
                end
            end  // end forever
        end
    endtask // read_fs
    ...
    task block_erase_32k;
        ...
        begin
            ...
            if ( write_protect(Address) == 1'b0 &&
               ...
               ERS_Time = Factory_Mode_Reg ? ERS_Count_BE32K_FM : ERS_Count_BE32K;
               fork
                   er_timer;
                   begin
    ...
    task block_erase;
        ...
        begin
            ...
            if ( write_protect(Address) == 1'b0 &&
               ...
               ERS_Time = Factory_Mode_Reg ? ERS_Count_BE_FM : ERS_Count_BE;
               fork
                   er_timer;
                   begin
                       for( i = 0; i < ERS_Time; i = i + 1 ) begin
    ...
    task sector_erase_4k;
        ...
        begin
            ...
            if ( write_protect(Address) == 1'b0 ) begin
               ...
               ERS_Time = Factory_Mode_Reg ? ERS_Count_SE_FM : ERS_Count_SE;
               fork
                   er_timer;
                   begin
                       for( i = 0; i < ERS_Time; i = i + 1 ) begin
    ...
    task chip_erase;
        ...
        begin
            ...
            if ( (Dis_CE == 1'b1 && WPSEL_Mode == 1'b0) || ( ( (WP_B_INT == 1'b0) || ( (SEC_Pro_Reg_BOT) && (SEC_Pro_Reg_TOP)&& (&SEC_Pro_Reg) ) ) && WPSEL_Mode == 1'b1) ) begin
                ...
                for ( i = 0;i<(Factory_Mode_Reg ? tCE_FM : tCE)/100;i = i + 1) begin
                    #100_000_000;
                end
    ...
    function time pgm_time_cal;
        ...
        begin
            ...
            if ( pgm_num == 1 ) begin
                ...
                pgm_time_cal = Factory_Mode_Reg ? tPP_FM : tPP;
            end
        end
    endfunction
    ...
    task read_4xio;
        ...
        begin
            ...
            if ( CMD_BUS == W4READ || (!READ4X_Mode && (CMD_BUS == RSTEN || CMD_BUS == RST) && EN4XIO_Read_Mode == 1'b1) )
                dummy_cycle(2);
